...
import { Search, Sparkles, Calendar, ChevronRight } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-4xl md:text-5xl font-bold">AI Impact Updates</h1>
        </div>
        <p className="text-lg text-muted-foreground">
          Weekly insights into AI's impact on finance and markets
        </p>
      </motion.div>
      ...
      {isLoading ? (
        ...
        >
          {filteredUpdates.map((update, index) => (
            ...
            >
              <Link to={`/ai-updates/${update.id}`} className="block group h-full">
        ...
        <div className="text-center py-12">
          <p className="text-muted-foreground text-lg">No AI updates found</p>
        </div>
      )}
    </div>
  );
